...
| Hook | 事件 | 用途 |
|------|------|------|
| `session-restore.sh` | `SessionStart` | 新会话/恢复时自动读取 Pipeline Status + 状态文件 |
| `context-refresh.sh` | `PreToolUse`（节流） | 周期性将 Pipeline Status 注入上下文 |
| `pre-compact-remind.sh` | `PreCompact` | 压缩前提醒 LLM 保存状态 |
| `progress-remind.sh` | `PostToolUse`（Write/Edit/MultiEdit） | 代码修改后提醒更新 EXPERIMENT_TRACKER.md |

> 重要：不要把恢复/提醒类 hook 挂到 `WebSearch` 或 `WebFetch` 上。否则网页搜索很容易返回空结果，只剩下注入的 reminder 文本。推荐把 `session-restore.sh` 放到 `SessionStart`，并把 `PreToolUse` matcher 限制为 `Bash|Read|Write|Edit|MultiEdit|Glob|Grep|Task` 这类代码/文件工具。
...
#!/bin/bash
# SessionStart hook: 新会话开始或恢复时自动恢复项目上下文
...
#!/bin/bash
# PreToolUse hook: 每 30 次代码/文件类工具调用刷新一次 Pipeline Status
...
可选自动化（仅 Claude Code）：
  session-restore.sh → 会话开始/恢复时自动读取状态
