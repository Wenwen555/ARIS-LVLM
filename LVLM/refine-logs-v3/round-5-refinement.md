...
- **Why the revised method still addresses it**: the method remains a selective-prediction gate over answer-conditioned evidence; the new changes only close the last protocol loopholes for swap-answer labeling workload and same-class control sampling.
- **Reviewer suggestions rejected as drift**: no benchmark expansion, no parser-heavy macro alignment subsystem, no extra explanation head, no extra trainable causal module.
...
- **Components removed or merged**: macro cue remains optional and stays out of the dominant claim; control details are handled by protocol constraints rather than new modules.
- **Reviewer suggestions rejected as unnecessary complexity**: no answer set expansion beyond top-1 answer and one deterministic competitor, no synthetic counterfactual video generation.
- **Why the remaining mechanism is still the smallest adequate route**: the paper still uses one retriever, one verifier, one clean EEA benchmark, and a few tightly specified held-out controls.

## Changes Made

### 1. Bound the swap-answer labeling workload
- **Reviewer said**: the deterministic `a'` rule may imply hidden annotation expansion.
- **Action**: define the EEA protocol to label exactly two answer-evidence pairs per selected question on the clean set: the emitted answer `a` and one deterministic competitor `a'` from the frozen beam.
- **Reasoning**: this keeps the swap diagnostic auditable without turning EEA into a multi-answer benchmark.
- **Impact on core method**: preserves the anti-confirmation check while keeping annotation cost modest and explicit.

### 2. Tighten same-class control sampling
- **Reviewer said**: same-class controls need to avoid near-duplicate leakage.
- **Action**: sample `E_K^{class}` only from different video IDs within the same procedural class, exclude any duplicate source clip or near-duplicate ASR/script overlap, and keep the same retrieval budget `K`.
- **Reasoning**: this makes the cross-video control a fair but truly non-instance-specific negative.
- **Impact on core method**: removes the last ambiguity in the visual-dependence identification test.
...
For the swapped-answer diagnostic, let `\mathcal{A}(q,V)` be the frozen generator’s beam or candidate list. Define the competing answer deterministically as the highest-scoring non-emitted candidate:
\[
a' = \arg\max_{\tilde a \in \mathcal{A}(q,V) \setminus \{a\}} p(\tilde a \mid q,V).
\]
On the clean EEA split, annotation is collected only for the pair `(q,a,E_K)` and the single competitor pair `(q,a',E_K)`, so the labeling workload is exactly two answer-evidence judgments per selected question. The swap margin is
\[
...
\]
Here `E_K^{same}` is formed by sampling clips from the same video whose timestamps are disjoint from every interval in `E_K` with exclusion margin `\epsilon`, and `E_K^{class}` is formed by clips from a different video ID in the same procedural class, excluding exact or near-duplicate clips under ASR/script overlap filtering. We then measure
\[
...
- On the clean swap subset, annotators additionally judge exactly one deterministic competitor `a'` for the same evidence.
...
- **Same-class control leaks near duplicates**: enforce different video IDs and ASR/script overlap filtering.
...
- **Data / annotation cost**: a few hundred EEA annotations plus timestamp validation, one deterministic competitor label on the clean swap subset, and partial double annotation.
