...
- **Why the revised method still addresses it**: the method remains a selective-prediction gate over answer-conditioned evidence; the new changes only identify whether the verifier truly depends on cited visual content and make the human evaluation protocol reliable enough to support that claim.
- **Reviewer suggestions rejected as drift**: no expansion into benchmark-heavy dataset work, no parser-heavy macro alignment subsystem, no extra explanation head.

## Simplicity Check
- **Dominant contribution after revision**: answer-conditioned evidence sufficiency verification with calibrated abstention and explicit visual-dependence identification controls.
- **Components removed or merged**: macro cue remains optional; broad unsupported-answer metrics remain proxy-only; the new controls are evaluation-time interventions over the same evidence interface rather than new trainable modules.
- **Reviewer suggestions rejected as unnecessary complexity**: no separate visual causal model, no generative counterfactual video synthesis, no expanded structured annotation taxonomy beyond entitled / not entitled.
- **Why the remaining mechanism is still the smallest adequate route**: the paper still needs only candidate answer generation, answer-conditioned clip retrieval, one verifier head, one human-auditable EEA benchmark, and two tightly scoped evidence corruption controls.

## Changes Made

### 1. Add explicit video-dependence controls
- **Reviewer said**: EEA + swap may still reward text-only compatibility rather than true visual entitlement.
- **Action**: add two control evidence sets on EEA-entitled cases: (i) random or timestamp-shuffled clips from the same video, and (ii) clips from a different video in the same class.
- **Reasoning**: if the verifier really checks entitlement from cited pixels, both the sufficiency score and the swapped-answer margin must collapse under these controls.
- **Impact on core method**: directly identifies whether the claim is about visual entitlement rather than textual plausibility.

### 2. Make EEA a registered protocol rather than an informal subset
- **Reviewer said**: if EEA is the primary target, its protocol reliability is part of the contribution.
- **Action**: pre-register the annotation prompt, define the exact input shown to annotators `(q, a, timestamps, clips)`, use binary labels `entitled / not entitled`, double-annotate a subset, and report agreement plus adjudication.
- **Reasoning**: this turns EEA from a vague clean set into an auditable benchmark slice that can support selective-prediction claims.
- **Impact on core method**: the verifier is now evaluated against a stable target rather than an underspecified notion of support.

### 3. Split EEA into dev/test and make thresholding risk-controlled
- **Reviewer said**: calibration and final evaluation should not leak through the same clean subset.
- **Action**: split EEA into dev for threshold selection and test for final reporting; choose `tau` on EEA-dev to satisfy a target risk level, then report achieved coverage and false-abstain on EEA-test.
- **Reasoning**: strengthens the selective-prediction framing without adding new modeling complexity.
- **Impact on core method**: the abstention threshold becomes a proper part of the evidence-entitlement protocol.
...
## Method Thesis
- **One-sentence thesis**: We reduce unsupported answering in procedural long-video QA by requiring every candidate answer to pass an answer-conditioned evidence sufficiency verifier, and we validate this gate with controls that test whether entitlement truly depends on the cited pixels.
...
7. On EEA examples, run control evidence replacements to verify the score depends on cited pixels
```
...
\[
E_K^{same} \sim \mathrm{ShuffleSameVideo}(V), \qquad
E_K^{class} \sim \mathrm{SampleSameClass}(\mathcal{D}\setminus V).
\]
Here `E_K^{same}` is formed by random clips or shuffled timestamps from the same video, and `E_K^{class}` is formed by clips from a different video in the same procedural class. We then measure
\[
...
\]
This is the paper’s core identification check for “entitlement requires the cited pixels.”
...
- They answer exactly: **“Does this evidence entitle this answer?”**
...
The paper’s novelty is not another confidence threshold. It is a minimal answer-conditioned entitlement test over cited visual evidence, with abstention tied to evidence sufficiency, with explicit anti-confirmation diagnostics, and with held-out controls that test whether the verifier actually needs the cited pixels. The method stays practical precisely because it refuses to make parser-level claims while still making the core causal story auditable.
...
### Claim 2: Entitlement depends on the cited pixels, not just text compatibility
- **Minimal experiment**: on EEA-entitled examples, replace `E_K` with `E_K^{same}` and `E_K^{class}`.
- **Baselines / ablations**: original evidence, same-video random/shuffled evidence, same-class cross-video evidence.
- **Metric**: score drop `Delta_vid`, swap-gap collapse, and resulting risk-coverage degradation.
- **Expected evidence**: `S_theta(q,a,E)` and `Delta_swap(E)` are high only for the real cited evidence, and collapse under both control replacements.
...
- **Must-prove claims**: verifier improves EEA-based selective prediction; entitlement depends on cited pixels; answer-conditioned retrieval and practical counterfactual negatives help.
- **Must-run ablations**: no verifier, confidence threshold, question-conditioned retrieval, no macro cue, no swap loss, no dropout negatives, same-video shuffled evidence, same-class cross-video evidence.
- **Critical datasets / metrics**: COIN/Ego4D subsets, `EEA_dev`, `EEA_test`, EEA, risk-coverage, false-abstain, `Delta_swap`, `Delta_vid`.
- **Highest-risk assumptions**: retriever quality, EEA labeling consistency, true score collapse under evidence corruption, transfer from clean EEA to broader data.
